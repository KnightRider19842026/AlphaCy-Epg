...
def generate_alpha_epg(days=4):
    # Ξεκινάμε από σήμερα (ή αύριο αν θες)
    start_date = datetime.datetime.now(datetime.timezone.utc).astimezone(datetime.timezone(datetime.timedelta(hours=3)))
    ...
    # Πρόγραμμα βάσει πραγματικών ωρών του Alpha Cyprus
    daily_schedule = [
        ("060000", "064500", "DEAL"),
        ("064500", "092000", "ALPHA ΚΑΛΗΜΕΡΑ"),
        ("092000", "110000", "BUONGIORNO"),
        ("110000", "135000", "ALPHA ΕΝΗΜΕΡΩΣΗ"),
        ("135000", "151500", "ΤΟ ΣΟΪ ΣΟΥ"),
        ("151500", "170000", "ΜΕ ΑΓΑΠΗ ΧΡΙΣΤΙΑΝΑ"),
        ("170000", "175500", "THE CHASE GREECE"),
        ("175500", "180000", "ALPHA NEWS"),
        ("180000", "190000", "ΝΑ Μ' ΑΓΑΠΑΣ"),
        ("190000", "200000", "DEAL"),
        ("200000", "210000", "ALPHA NEWS"),
        ("210000", "222000", "ΑΓΙΟΣ ΕΡΩΤΑΣ"),      # Δευ-Πέμ
        ("222000", "230000", "Η ΓΗ ΤΗΣ ΕΛΙΑΣ"),    # Δευ-Τρί
        ("230000", "000000", "ΝΑ Μ' ΑΓΑΠΑΣ (Ε)"),
    ]
    ...
    for d in range(days):
        ...
        for start_time, stop_time, title in daily_schedule:
            ...
            # Αν το stop είναι 000000, πάμε στην επόμενη μέρα
            if stop_time == "000000":
                stop_date = current_date + datetime.timedelta(days=1)
                stop = f"{stop_date.strftime('%Y%m%d')}000000 +0300"
            else:
    ...
    # Αφαιρούμε την πρώτη γραμμή <?xml ...> για να μην διπλοεμφανίζεται
